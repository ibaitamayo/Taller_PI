...
##TIENE SENTIDO??
df_ff %>% mutate(high_outcome=factor(high_outcome,levels=c(0,1), labels=c("Low expectancy","High expectancy")))->df_ff
...
```{r finalfit_bin_all}
num_cols <- df_ff%>% names()
explanatory_all <- setdiff(num_cols, "high_outcome")
dependent <- "high_outcome"



df_ff %>% summary_factorlist(dependent, explanatory_all, p = TRUE) %>% regulartable() %>% theme_zebra()



df %>% filter(Life_expectancy>72)->df_select_bias

df %>%mutate(BMI=ifelse(Population>1251656,BMI,NA))->df_measure_bias
```
