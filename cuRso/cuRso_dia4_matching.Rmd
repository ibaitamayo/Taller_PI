...
```{r}
df<-read.csv("datos/diabetic_data.csv")

df %>% mutate(complication_bin=ifelse(readmitted == "<30","1","0"))->dff

explanatory=c("age_mid", "gender", "race", "time_in_hospital", "num_lab_procedures", "num_medications", "number_diagnoses", "insulin", "change", "payer_code")

dff %>% summary_factorlist(dependent ="complication_bin",explanatory,p=TRUE) %>% flextable::regulartable() %>% flextable::theme_zebra()


c("age_mid", "race", "time_in_hospital", "num_lab_procedures", "num_medications", "number_diagnoses", "insulin", "change", "payer_code")

modelo_completo<-glm(as.numeric(complication_bin)~age_mid+ race+ time_in_hospital+ num_lab_procedures+ num_medications+ number_diagnoses+ insulin+ change+ payer_code,family = binomial,data = dff)

dff %>%
  mutate(insulin=factor(ifelse(insulin=="No",
                               0,
                               ifelse(insulin=="Down",
                                      1,
                                      ifelse(insulin=="Steady",2,3))),
                        levels=c(0:3), labels = c("No","poco", "medio", "mucho")))->dff
...
modelo_adj1<-glm(as.numeric(complication_bin)~age_mid+ race+ time_in_hospital+ num_medications+ number_diagnoses+ insulin+ change+ payer_code,family = binomial,data = dff)
summary(modelo_adj1)




modelo_adj2<-glm(as.numeric(complication_bin)~ race+ time_in_hospital+ num_medications+ number_diagnoses+ insulin+ change+ payer_code,family = binomial,data = dff)
summary(modelo_adj2)
...
df0 <- read.csv("datos/diabetic_data.csv")
...
# 1) Carga (coloca smoker.csv junto al Rmd/notebook)
df0 <- read_csv("datos/smoker.csv", show_col_types = FALSE)

# 2) Limpieza de nombres permitida (no cambia estos, pero cumplo la regla)
names(df0) <- chartr(" /-","___", str_squish(str_trim(names(df0))))

# 3) Comprobación de columnas esperadas
stopifnot(all(c("smoker","treatment","outcome") %in% names(df0)))

# 4) Preparación
df <- df0 %>%
  mutate(across(c(smoker, treatment, outcome), ~as.integer(.))) %>%
  tidyr::drop_na(smoker, treatment, outcome)

# 5) Tasas pre-matching
rate_pre <- with(df, tapply(outcome, treatment, mean))
pre_tbl  <- tibble(context="Pre",
                   group=c("Control","Treated"),
                   rate=c(rate_pre["0"], rate_pre["1"]))

# 6) Fórmula de PS: trat ~ confusores (solo 'smoker' en este dataset)
form_ps <- treatment ~ smoker

# 7) Matching NN 1:1
set.seed(1)
m11 <- matchit(form_ps, data=df, method="nearest", ratio=1)

d11 <- match.data(m11)
rate_post11 <- with(d11, tapply(outcome, treatment, mean))
post11_tbl  <- tibble(context="Post_1to1",
                      group=c("Control","Treated"),
                      rate=c(rate_post11["0"], rate_post11["1"]))

# 8) Matching NN 1:2
set.seed(2)
m12 <- matchit(form_ps, data=df, method="nearest", ratio=2)

d12 <- match.data(m12)
rate_post12 <- with(d12, tapply(outcome, treatment, mean))
post12_tbl  <- tibble(context="Post_1to2",
                      group=c("Control","Treated"),
                      rate=c(rate_post12["0"], rate_post12["1"]))

# 9) Balance: tablas y LOVE plots
bal_11 <- bal.tab(m11, un=TRUE, m.threshold=0.1)
bal_12 <- bal.tab(m12, un=TRUE, m.threshold=0.1)

print(bal_11); print(bal_12)

love.plot(m11, binary="std", abs=TRUE, thresholds=c(m=.1),
          var.order="unadjusted", title="Equilibrio SMD - Matching 1:1")
love.plot(m12, binary="std", abs=TRUE, thresholds=c(m=.1),
          var.order="unadjusted", title="Equilibrio SMD - Matching 1:2")

# 10) Resumen de tasas pre/post para interpretar en clase
rates <- bind_rows(pre_tbl, post11_tbl, post12_tbl)
print(rates)
